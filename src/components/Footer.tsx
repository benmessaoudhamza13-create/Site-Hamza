...
const pill =
  "group flex items-center gap-2 rounded-full border rule px-3 py-1.5 label transition-colors duration-200";

export default function Footer() {
  const t = useT();

  return (
    <footer className="border-t rule bg-paper-2/60">
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-6 py-10 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
          {/* Ville avec croquis au survol */}
          <span className="tip label inline-flex cursor-default items-center gap-2 transition-colors duration-200 hover:text-accent">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <a className={`${pill} hover:border-accent hover:text-accent`} href="mailto:hamza.ben-messaoud@hec.ca">
            <Mail size={14} strokeWidth={1.75} aria-hidden />
            {t(ui.contact.email)}
          </a>
          <a className={`${pill} hover:border-accent hover:text-accent`} href="tel:+14386301061">
            <Phone size={14} strokeWidth={1.75} aria-hidden className="ring" />
            438 630-1061
          </a>
          <a
            className={`${pill} hover:border-[#0a66c2] hover:text-[#0a66c2]`}
